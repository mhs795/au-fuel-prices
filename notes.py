...
SECTIONS = [
    ...
    ("SUMMARY CHARTS TAB", [
        ("What it is", "Twenty-one native Excel charts - every series at daily (two windows), monthly, quarterly and annual frequency, plus one two-scale chart. They are real charts pointing at cells on the data tabs, not pictures, so they redraw whenever the workbook is rebuilt and can be copied straight into a document or deck."),
        ("Daily charts are windowed", "Each series has two daily charts - the last two years and the last five. Full daily history is on the daily tabs; twenty-eight years of daily spot prices in a chart this size is a solid block of ink. The windows are measured back from each series' own last date, so the gas ones end a few weeks earlier than the rest."),
        ("Petrol charts have a fixed scale", "The four petrol TGP and four petrol retail charts are pinned to 100-300 c/L rather than auto-scaled, so wholesale and retail, and every frequency, can be read against one another without rescaling by eye. The cost is that anything outside that band is drawn off the top or bottom of the plot: the TGP daily charts clip 11 days above 300 c/L in late March and April 2026 (peak 325.9), and on full-history TGP the early years sit below the floor - 31 months to May 2020, 10 quarters to 2020-Q2, and calendar 2004. Retail is entirely inside the band except 15 days at the April-May 2020 trough. Check the data tab before quoting a peak or a trough off these charts. Electricity and gas are left auto-scaled; their spikes are order-of-magnitude, and a fixed range would either flatten them or cut them off."),
