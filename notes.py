...
SECTIONS = [
    ...
    ("SUMMARY CHARTS TAB", [
        ...
        ("Daily charts are windowed", "Each series has two daily charts - the last two years and the last five - and petrol has two more, one for 2026 and one for 2025, so a year can be read against the year before it on the same fixed axis. The current-year chart is labelled with the date it runs to, because it is a part year. Full daily history is on the daily tabs; twenty-eight years of daily spot prices in a chart this size is a solid block of ink. The windows are measured back from each series' own last date, so the gas ones end a few weeks earlier than the rest."),
