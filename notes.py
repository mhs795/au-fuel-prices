...
SECTIONS = [
    ...
    ("DAILY / MONTHLY / ANNUAL TABS", [
        ("Structure", "Every series appears three times - daily, monthly and annual. The daily tab is the source of truth; the monthly and annual tabs are averages of it. Columns are identical across the three frequencies so a formula written against one works against the others."),
        ("Averaging rule", "Monthly and annual figures are simple means of the daily values present in that period. Days with no value are skipped, never treated as zero, so a source gap pulls the average toward the days that do exist rather than dragging it down."),
        ("Electricity is different", "Electricity monthly and annual demand-weighted prices are accumulated from the underlying 5-minute and 30-minute intervals, NOT averaged from the daily figures. An average of daily averages is not a demand-weighted average and would misprice volatile months, where a few high-demand, high-price intervals do most of the work. The min and max columns are likewise the true extremes over the period, not averages of daily extremes."),
        ("n_days column", "Every monthly and annual row carries n_days - how many days that average actually rests on. Use it to spot partial periods. The current month and current year are always partial, and periods with a source gap will read low."),
        ...
        ("Calendar, not financial", "Annual means calendar year. Financial-year figures are not included; they can be added if wanted."),
