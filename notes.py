...
SECTIONS = [
    ...
    ("SUMMARY CHARTS TAB", [
        ("What it is", "Twenty-six native Excel charts - every series at monthly, quarterly and annual frequency, daily as well for gas and petrol, plus four two-scale electricity-and-gas charts, one per state. Electricity has no daily chart: half-hourly spot averaged to a day is too spiky to read at chart size, and the daily figures are still on the Electricity daily tab. They are real charts pointing at cells on the data tabs, not pictures, so they redraw whenever the workbook is rebuilt and can be copied straight into a document or deck."),
        ...
        ("Colour is consistent", "A series keeps the same colour across every chart it appears in, so NSW is the same blue on the monthly, quarterly and annual electricity charts. The palette was checked for colour-vision-deficiency separation."),
