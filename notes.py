...
SECTIONS = [
    ...
    ("SUMMARY CHARTS TAB", [
        ("What it is", "Thirty-six native Excel charts, laid out in headed sections - electricity and gas by state, then electricity, gas, wholesale petrol and retail petrol, each under its own heading. Every series is charted at monthly, quarterly and annual frequency, daily as well for gas and petrol, plus four two-scale electricity-and-gas charts, one per state, and a block of single-state daily petrol charts for 2025. Electricity has no daily chart: half-hourly spot averaged to a day is too spiky to read at chart size, and the daily figures are still on the Electricity daily tab. They are real charts pointing at cells on the data tabs, not pictures, so they redraw whenever the workbook is rebuilt and can be copied straight into a document or deck."),
        ("Daily charts are windowed", "Each series has two daily charts - the last two years and the last five - and petrol has two more, one for 2026 and one for 2025, plus ten single-state charts for 2025 (see below). Those two carry tighter axes of their own - 125-350 c/L for 2026, 125-200 for 2025 - so a single year's movement is legible instead of being flattened into the full band. The two years are therefore NOT on a common scale: compare the shapes, and read the numbers off the axis before comparing levels between them. The current-year chart is labelled with the date it runs to, because it is a part year. Full daily history is on the daily tabs; twenty-eight years of daily spot prices in a chart this size is a solid block of ink. The windows are measured back from each series' own last date, so the gas ones end a few weeks earlier than the rest."),
        ...
        ("One state per chart", "At the end of the wholesale and retail petrol sections there is a block of daily 2025 charts carrying a single state each - the seven capital-city terminal gate prices, and retail ULP for the three states with daily pump data (NSW, QLD, WA). They exist because the all-states charts are where day-to-day reading breaks down: the capitals track each other within a couple of cents at the terminal gate, and three retail discount cycles running out of phase read as noise rather than as three cycles. Within each block the states share one axis - 150-190 c/L for the terminal gate charts, 155-200 for retail - so the charts can be read side by side; both bands are checked against the data on every build and neither crops anything. They are NOT on the same scale as each other, or as the 75-350 band, so read the axis before comparing levels across blocks. The terminal gate charts carry both petrol and diesel; the retail ones carry ULP only, and the other grades are on the retail data tabs."),
        ("Colour is consistent", "A series keeps the same colour across every chart it appears in, so NSW is the same blue on the monthly, quarterly and annual electricity charts, and NSW retail keeps its colour on its own single-state chart. The palette was checked for colour-vision-deficiency separation."),
